...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.large_test_image = customtkinter.CTkImage(Image.open(os.path.join(image_path, "large_test_image.png")),
                                                       size=(500, 150))
        self.image_icon_image = customtkinter.CTkImage(Image.open(os.path.join(image_path, "play_light.png")),
                                                       size=(20, 20))
        ...
        self.add_station_image = customtkinter.CTkImage(
            light_image=Image.open(os.path.join(image_path, "station_dark.png")),
        ...
        self.navigation_frame_label = customtkinter.CTkLabel(self.navigation_frame, text="  Умный дом",
        ...
        self.frame_2_button = customtkinter.CTkButton(self.navigation_frame, corner_radius=0, height=40,
                                                      border_spacing=10, text="Сценарии",
                                                      fg_color="transparent", text_color=("gray10", "gray90"),
                                                      hover_color=("gray70", "gray30"),
        ...
        self.frame_4_button = customtkinter.CTkButton(self.navigation_frame, text="Обновить",
        ...
        def all1():
            ...
            def switch_event(a):
                ...
                data1 = '''{"devices": [{"id": "''' + a[
                    0] + '''","actions": [{"type": "devices.capabilities.on_off","state": {"instance": "on","value": ''' + \
                        switch_var_sp[a[1]].get() + '''}}]}]}'''
                r = requests.post(url, headers=headers, data=data1)
                if (json.loads(r.text).get("devices")[0].get("capabilities")[0].get("state").get("action_result").get(
                        "status") == 'ERROR') and (a[1] not in ofline_sp):
                    label2_sp.append(
                        customtkinter.CTkLabel(master=frame_sp[a[2]], justify=customtkinter.LEFT, text="offline"))
            ...
            for i in range(len(info['devices'])):
                ...
                label_sp.append(
                    customtkinter.CTkLabel(master=frame_sp[-1], justify=customtkinter.LEFT, text=info['name'][i]))
                ...
                if (info['info'][i] == 'devices.types.light') or (info['info'][i] == 'devices.types.socket'):
                    ...
                    switch_sp.append(
                        customtkinter.CTkSwitch(master=frame_sp[-1], text="ON/OFF", command=for_switch_sp[-1],
                                                variable=switch_var_sp[-1], onvalue="true", offvalue="false"))
        ...
        def all2():
            ...
            for i in range(len(info["scenarios"][0])):
                ...
                label_sp.append(customtkinter.CTkLabel(master=frame_sp[-1], justify=customtkinter.LEFT,
                                                       text=info["scenarios"][1][i]))
                frame_sp[i].grid(row=y, column=x, padx=20, pady=10, sticky="nsew")
                label_sp[i].pack(pady=0, padx=0)
                button_sp.append(customtkinter.CTkButton(master=frame_sp[-1], text="", image=self.image_icon_image,
                                                         command=for_button_sp[-1]))
